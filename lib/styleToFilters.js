...
function styleToFilters(style) {
  ...
  if (style && style.metadata && style.metadata[metadata_key]) {
    Object.keys(style.metadata[metadata_key]).forEach(sourceLayer => {
      properties_for_layers[sourceLayer] = style.metadata['mapbox:retain'][sourceLayer];
    })
  }
  ...
  Object.keys(properties_for_layers).forEach(sourceLayer => {
    if (!layers[sourceLayer]) {
      throw new Error("Invalid source layer '" + sourceLayer + "' in metadata key '"+ metadata_key + "'");
    }
  })
