 ...
 * @param {Object} options.properties - which properties you want keep 
 * @example
 * const shaver = require('@mapbox/vtshaver');
 * const options = {properties: {road: ['name_en']}};
 * const style = require('/path/to/style.json');
...
function styleToFilters(style, options) {


  var layers = {};
  // collect sources
  var properties_for_layers = {};

  // - collect source from style meta
  const metadata_key = 'mapbox:retain';
  if (style && style.metadata && style.metadata[metadata_key]) {
    collectSourceProperty(style.metadata[metadata_key], properties_for_layers);
  }
  // - collect source from options.properties
  if (options && typeof options === 'object' && options.properties) {
    collectSourceProperty(options.properties, properties_for_layers);
  }

  function collectSourceProperty(obj, properties) {
    if (typeof obj === 'object') {
      Object.keys(obj).forEach(sourceLayer => {
        properties[sourceLayer] = properties[sourceLayer] || [];
        properties[sourceLayer] = properties[sourceLayer].concat(obj[sourceLayer]);
      })
    }
  }
  ...
  // validate the properties_for_layers keys
  Object.keys(properties_for_layers).forEach(sourceLayer => {
    if (!layers[sourceLayer]) {
      delete properties_for_layers[sourceLayer];
    }
  })

  // remove duplicate propertys and fix choose all the properties in layers[i].properties
  Object.keys(layers).forEach(layerId => {
    let properties = layers[layerId].properties;
    // add properties from source
    if (properties_for_layers[layerId]) {
      properties = properties.concat(properties_for_layers[layerId]);
    }
...
if (require.main === module) {
  ...
  if (process.argv.length === 3 || process.argv.length === 4) {
    ...
    console.log(JSON.stringify(styleToFilters(style, options), '\t', 2));
