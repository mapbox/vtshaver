    ...
    for (var i = 0; i < style.layers.length; i++) {
      ...
      if (layerName) {
        ...
        ['paint', 'layout'].forEach(item => {
          ...
          itemObject && propertyLoop(itemObject, layers[layerName].properties);
        });
      }
    }
  }
...
function propertyLoop(propertyObj, properties) {
  Object.keys(propertyObj).forEach(key => {
    ...
    if (typeof value === 'string') {
      ...
      if (styleSpec.expression.isExpression(value)) {
        // TODO: new we implement this by ourself in vtshave
        // we need to talk with style spec team to see 
        // if there have a origin way to get used property from styleSpec
        // to make this can be synchronized with the expression update
        getPropertyFromExpression(value, properties);
        // 
      } else {
        // otherwise continual loop;
        propertyLoop(value, properties);
      }
    }
  })
}
