      ...
      if (layerName) {
        ...
        if (layers[layerName]) {
          ...
          if (layers[layerName].filters === true || !style.layers[i].filter) {
            ...
            layers[layerName].filters.push(replaceNoOpExpressions(style.layers[i].filter));
          }
        } else {
          // otherwise create the layer & filter array, with min/max zoom
          layers[layerName] = {};
          layers[layerName].filters = style.layers[i].filter ? ['any', replaceNoOpExpressions(style.layers[i].filter)] : true;
...
function reducableToSimpleTrueFalse(exp) {
  if (typeof exp === 'boolean') return true;
  else if (exp instanceof Array) {
    switch (exp[0]) {
      case 'literal':
      case 'boolean':
        return exp.slice(1).every(sub => reducableToSimpleTrueFalse(sub));
      case 'to-boolean':
        return ['', 0, false, null, NaN].includes(exp[1]) || exp.slice(1).every(sub => reducableToSimpleTrueFalse(sub));
      return false;
    }
  }
  else return false;
...
function replaceNoOpExpressions(exp) {
  if (exp instanceof Array) {
    ...
    return newExp.includes('noop') || (newExp[0] === 'case' && newExp.slice(1).every(sub => reducableToSimpleTrueFalse(sub))) ? ['literal', true] : newExp;
  }
}
