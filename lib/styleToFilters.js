...
function styleToFilters(style) {
  ...
  //   throw new Error('Param "style" must an object with the "layers" property');
  // }
    ...
    for (var i = 0; i < style.layers.length; i++) {
      ...
      if (layerName) {
        ...
        ['paint', 'layout'].forEach(item => {
          ...
          itemObject && getPropertyLoop(itemObject, layers[layerName].properties);
        });
      }
    }
  }
...
function getPropertyLoop(propertyObj, properties) {
  Object.keys(propertyObj).forEach(key => {
    ...
    if (typeof value === 'string') {
      ...
      if (styleSpec.expression.isExpression(value)) {
        ...
        getPropertyLoop(value, properties);
      }
    }
  })
}
...
function getPropertyFromExpression(exp, properties) {
  // Now we only need to care about the below expression:
  // 1. The array start with the "get": ["get", string], 
  //    but do not have the thrid member in the Array, for example we need to pass the following one: ["get", string, Object]
  // 2. The array start with the "has": ["has", string]
  //    As same as 1, we need ignore the Array with the 3rd Object param, like: ["has", string, Object]
  // 3. The array with "properties"(["properties"]), means we need to keep all the properties.
  // 4. The array with "feature-state"(["feature-state", string]), we need to keep the property
