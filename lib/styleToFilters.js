    ...
    for (var i = 0; i < style.layers.length; i++) {
      ...
      if (layerName) {
        ...
        if (layers[layerName]) {
          ...
          layers[layerName] = {};
          layers[layerName].filters = style.layers[i].filter && !isNoOpFilter(style.layers[i].filter) ? ['any', style.layers[i].filter] : true;
...
function isNoOpFilter(exp) {
  if (typeof exp === 'string') return ['pitch', 'distance-from-center'].includes(exp);
  else if (exp instanceof Array) return exp.some(sub => isNoOpFilter(sub));
  else return false;
}
