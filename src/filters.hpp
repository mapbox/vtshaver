...
class Filters : public Nan::ObjectWrap {
  public:
    ...
    using filter_values_type = std::tuple<filter_value_type, filter_properties, zoom_type, zoom_type>;
    ...
    void add_filter(filter_key_type&& key, filter_value_type&& filter, filter_properties properties, zoom_type minzoom, zoom_type maxzoom) {
        // add a new key/value pair, with the value equaling a tuple 'filter_values_type' defined above
        filters.emplace(key, std::make_tuple(std::move(filter), properties, minzoom, maxzoom));
    }
